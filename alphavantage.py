...
for i in range(len(initial.index)-200,1,-1):
    ...
    three = int(initial['open'][i] - initial['bband1'][i] <= 0.1) #bollinger bands
    total = one + two + three

    newOne = int(initial['ema1'][i] >= initial['ema2'][i])
    newTwo = int(initial['rsi'][i] >= 90)
    newThree = int(initial['bband1'][i] - initial['open'][i] <= 0.1)
    ...
    stopprofit = float(initial['open'][i-1]) * (1+0.0001)
    stoploss = float(initial['open'][i-1]) * (1-0.0001)
